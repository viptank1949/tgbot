<?php /*a:2:{s:66:"/www/wwwroot/11.fanlipt.com/app/admin/view/uploadconfig/index.html";i:1638805058;s:64:"/www/wwwroot/11.fanlipt.com/app/admin/view/common/container.html";i:1643093912;}*/ ?>
<!doctype html>
<html lang="en">
<head>
<meta charset="utf-8" />
...
<div v-if="search_visible" id="search" class="search">
	<el-form ref="form" size="small" :model="searchData" inline>
		<el-form-item label="覆盖原图">
			<el-select style="width:150px" v-model="searchData.upload_replace" filterable clearable placeholder="请选择">
				<el-option key="0" label="开启" value="1"></el-option>
				<el-option key="1" label="关闭" value="0"></el-option>
			</el-select>
		</el-form-item>
		<el-form-item label="生成缩略图">
			<el-select style="width:150px" v-model="searchData.thumb_status" filterable clearable placeholder="请选择">
				<el-option key="0" label="开启" value="1"></el-option>
				<el-option key="1" label="关闭" value="0"></el-option>
			</el-select>
		</el-form-item>
		<el-form-item label="缩放类型">
			<el-select style="width:150px" v-model="searchData.thumb_type" filterable clearable placeholder="请选择">
...
<el-table :row-class-name="rowClass" @selection-change="selection"  @row-click="handleRowClick"  row-key="id"  :header-cell-style="{ background: '#eef1f6', color: '#606266' }" v-loading="loading"  ref="multipleTable" border class="eltable" :data="list"  style="width: 100%">
	...
	<el-table-column align="center"  property="title" label="配置名称" show-overflow-tooltip width="">
	</el-table-column>
	<el-table-column align="center"  property="upload_replace" label="覆盖原图" show-overflow-tooltip width="">
		<template slot-scope="scope">
			<el-switch @change="updateExt(scope.row,'upload_replace')" :active-value="1" :inactive-value="0" v-model="scope.row.upload_replace"></el-switch>
		</template>
	</el-table-column>
	<el-table-column align="center"  property="thumb_status" label="生成缩略图" show-overflow-tooltip width="">
		<template slot-scope="scope">
			<el-switch @change="updateExt(scope.row,'thumb_status')" :active-value="1" :inactive-value="0" v-model="scope.row.thumb_status"></el-switch>
		</template>
	</el-table-column>
	<el-table-column align="center"  property="thumb_width" label="缩略图宽" show-overflow-tooltip width="">
	</el-table-column>
	<el-table-column align="center"  property="thumb_height" label="缩略图高" show-overflow-tooltip width="">
	</el-table-column>
	<el-table-column align="center"  property="thumb_type" label="缩放类型" show-overflow-tooltip width="">
		<template slot-scope="scope">
			<span v-if="scope.row.thumb_type == '1'" >等比例缩放</span>
			<span v-if="scope.row.thumb_type == '2'" >缩放后填充</span>
			<span v-if="scope.row.thumb_type == '3'" >居中裁剪</span>
			<span v-if="scope.row.thumb_type == '4'" >左上角裁剪</span>
			<span v-if="scope.row.thumb_type == '5'" >右下角裁剪</span>
			<span v-if="scope.row.thumb_type == '6'" >固定尺寸缩放</span>
		</template>
	</el-table-column>
	<el-table-column :fixed="ismobile()?false:'right'" label="操作" align="center" width="180">
		<template slot-scope="scope">
			<div v-if="scope.row.id">
				<el-button v-if="checkPermission('/admin/Uploadconfig/update.html','<?php echo implode(",",session("admin.access")); ?>','<?php echo session("admin.role_id"); ?>',[1])" size="mini" icon="el-icon-edit" type="primary" @click="update(scope.row)" >修改</el-button>
				<el-button v-if="checkPermission('/admin/Uploadconfig/delete.html','<?php echo implode(",",session("admin.access")); ?>','<?php echo session("admin.role_id"); ?>',[1])" size="mini" icon="el-icon-delete" type="danger" @click="del(scope.row)" >删除</el-button>
			</div>
		</template>
	</el-table-column>
</el-table>
...
<script src="/components/admin/uploadconfig/add.js?v=<?php echo rand(1000,9999)?>"></script>
<script src="/components/admin/uploadconfig/update.js?v=<?php echo rand(1000,9999)?>"></script>
<script src="/components/admin/uploadconfig/detail.js?v=<?php echo rand(1000,9999)?>"></script>
<script>
new Vue({
	el: '#app',
	components:{
	...
	data: function() {
		return {
			...
			searchData:{},
			button_group:[
				{name:'添加',color:'success',access:'/admin/Uploadconfig/add.html',icon:'el-icon-plus',disabled:'',clickname:'add'},
				{name:'修改',color:'primary',access:'/admin/Uploadconfig/update.html',icon:'el-icon-edit',disabled:'single',clickname:'update'},
				{name:'删除',color:'danger',access:'/admin/Uploadconfig/delete.html',icon:'el-icon-delete',disabled:'multiple',clickname:'del'},
				{name:'查看详情',color:'info',access:'/admin/Uploadconfig/detail.html',icon:'el-icon-view',disabled:'single',clickname:'detail'},
			],
			loading: false,
			page_data: {
				limit: 20,
	...
	methods:{
		index(){
			...
			this.loading = true
			axios.post(base_url + '/Uploadconfig/index',param).then(res => {
				if(res.data.status == 200){
		...
		updateExt(row,field){
			if(row.id){
				axios.post(base_url + '/Uploadconfig/updateExt',{id:row.id,[field]:row[field]}).then(res => {
					if(res.data.status == 200){
		...
		update(row){
			...
			axios.post(base_url + '/Uploadconfig/getUpdateInfo',{id:id}).then(res => {
				if(res.data.status == 200){
		...
		del(row){
			this.$confirm('确定操作吗?', '提示', {
				...
				axios.post(base_url + '/Uploadconfig/delete',{id:ids}).then(res => {
					if(res.data.status == 200){
						this.$message({message: res.data.msg, type: 'success'})
						this.index()
					}else{
