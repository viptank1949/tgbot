<?php /*a:2:{s:58:"/www/wwwroot/11.fanlipt.com/app/admin/view/mass/index.html";i:1646989184;s:64:"/www/wwwroot/11.fanlipt.com/app/admin/view/common/container.html";i:1643093912;}*/ ?>
<!doctype html>
<html lang="en">
<head>
<meta charset="utf-8" />
...
<el-card shadow="never" class="card" style="min-height:650px;">
	<el-form size="small" ref="form" :model="form" :rules="rules" label-width="90px" >
				...
				<el-row >
					<el-col :span="24">
						<el-form-item label="群发分类" prop="type">
							<el-select multiple style="width:100%" v-model="form.fenlei" filterable clearable placeholder="请选择群发分类">
...
new Vue({
	...
	mounted(){
		axios.post(base_url + '/Mass/getInfo').then(res => {
			if(res.data.status == 200){
	...
	methods: {
		submit(){
			this.$refs['form'].validate(valid => {
				if(valid) {
					this.loading = true
					axios.post(base_url + '/Mass/index',this.form).then(res => {
						if(res.data.status == 200){
