<?php /*a:2:{s:58:"/www/wwwroot/11.fanlipt.com/app/admin/view/user/index.html";i:1647060832;s:64:"/www/wwwroot/11.fanlipt.com/app/admin/view/common/container.html";i:1643093912;}*/ ?>
<!doctype html>
<html lang="en">
<head>
<meta charset="utf-8" />
...
<div v-if="search_visible" id="search" class="search">
	<el-form ref="form" size="small" :model="searchData" inline>
		<el-form-item label="用户uid">
			<el-input id="tguid" v-model="searchData.tguid"  style="width:150px;" placeholder="请输入用户uid"></el-input>
		</el-form-item>
...
<el-table :row-class-name="rowClass" @selection-change="selection"  @row-click="handleRowClick"  row-key="id"  :header-cell-style="{ background: '#eef1f6', color: '#606266' }" @sort-change='sortChange' v-loading="loading"  ref="multipleTable" border class="eltable" :data="list"  style="width: 100%">
	...
	<el-table-column align="center"  property="tguid"  label="用户uid" show-overflow-tooltip width="">
	</el-table-column>
	...
	<el-table-column :fixed="ismobile()?false:'right'" label="操作" align="center" width="180">
		<template slot-scope="scope">
			<div v-if="scope.row.id">
				<el-button v-if="checkPermission('/admin/User/update.html','<?php echo implode(",",session("admin.access")); ?>','<?php echo session("admin.role_id"); ?>',[1])" size="mini" icon="el-icon-edit" type="primary" @click="update(scope.row)" >修改</el-button>
				<el-button v-if="checkPermission('/admin/User/delete.html','<?php echo implode(",",session("admin.access")); ?>','<?php echo session("admin.role_id"); ?>',[1])" size="mini" icon="el-icon-delete" type="danger" @click="del(scope.row)" >删除</el-button>
			</div>
		</template>
	</el-table-column>
</el-table>
...
<script src="/components/admin/user/add.js?v=<?php echo rand(1000,9999)?>"></script>
<script src="/components/admin/user/update.js?v=<?php echo rand(1000,9999)?>"></script>
<script src="/components/admin/user/detail.js?v=<?php echo rand(1000,9999)?>"></script>
<script>
new Vue({
	el: '#app',
	components:{
	...
	data: function() {
		return {
			...
			searchData:{},
			button_group:[
				{name:'添加',color:'success',access:'/admin/User/add.html',icon:'el-icon-plus',disabled:'',clickname:'add'},
				{name:'修改',color:'primary',access:'/admin/User/update.html',icon:'el-icon-edit',disabled:'single',clickname:'update'},
				{name:'删除',color:'danger',access:'/admin/User/delete.html',icon:'el-icon-delete',disabled:'multiple',clickname:'del'},
				{name:'查看详情',color:'info',access:'/admin/User/detail.html',icon:'el-icon-view',disabled:'single',clickname:'detail'},
			],
			loading: false,
			page_data: {
				limit: 20,
	...
	methods:{
		index(){
			...
			this.loading = true
			axios.post(base_url + '/User/index',param).then(res => {
				if(res.data.status == 200){
		...
		updateExt(row,field){
			if(row.id){
				axios.post(base_url + '/User/updateExt',{id:row.id,[field]:row[field]}).then(res => {
					if(res.data.status == 200){
		...
		update(row){
			...
			axios.post(base_url + '/User/getUpdateInfo',{id:id}).then(res => {
				if(res.data.status == 200){
		...
		del(row){
			this.$confirm('确定操作吗?', '提示', {
				...
				axios.post(base_url + '/User/delete',{id:ids}).then(res => {
					if(res.data.status == 200){
						this.$message({message: res.data.msg, type: 'success'})
						this.index()
					}else{
