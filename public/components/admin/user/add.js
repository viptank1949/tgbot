	...
	template: `
		<el-dialog title="添加" width="600px" class="icon-dialog" :visible.sync="show" @open="open" :before-close="closeForm" append-to-body>
			<el-form :size="size" ref="form" :model="form" :rules="rules" :label-width=" ismobile()?'90px':'16%'">
				<el-row >
					<el-col :span="24">
						<el-form-item label="用户uid" prop="tguid">
							<el-input  v-model="form.tguid" autoComplete="off" clearable  placeholder="请输入用户uid"></el-input>
						</el-form-item>
					</el-col>
				</el-row>
				<el-row >
	...
	methods: {
		...
		submit(){
			this.$refs['form'].validate(valid => {
				if(valid) {
					this.loading = true
					axios.post(base_url + '/User/add',this.form).then(res => {
